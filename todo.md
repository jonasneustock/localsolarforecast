...
- [ ] Define scope boundary (MVP = `/estimate`, `/clearsky`; P1 = `/history`, multi-plane).
- [ ] Select licenses (Apache-2.0 for our code; respect pvlib license).
- [ ] Repo bootstrap:
...
- [ ] Decide API compatibility target (HA uses Forecast.Solar public endpoints/fields).
- [ ] Add disclaimer (no affiliation; interface-compatible only).

---

## P0 — System architecture (design freeze)

- [ ] Confirm target stack: Python 3.11, FastAPI, uvicorn, pvlib, Redis, Docker, docker-compose.
...
- [ ] Timezone policy: default `Europe/Berlin`, override via env.
- [ ] Observability baseline: `/health`, structured logs, Prometheus metrics (P1).

---

## P0 — API contract (HA compatibility)

- [ ] Implement endpoints with **Forecast.Solar-shaped** responses:
  - [ ] `GET /estimate/{lat}/{lon}/{declination}/{azimuth}/{kwp}`
  - [ ] `GET /clearsky/{lat}/{lon}/{declination}/{azimuth}/{kwp}`
- [ ] Response schema (must match HA expectations):
  ```json
  {
    "result": {
  ...
  ```
- [ ] Conventions:
  - [ ] Timestamps in **local time**, string keys.
  - [ ] Resolution default 60-min; query `?time=15m` accepted (round to available weather cadence).
  - [ ] Azimuth convention: **0 = South, +E, −W** (mirror Forecast.Solar/HA).
  - [ ] Declination = tilt in degrees from horizontal.
...
- [ ] Project tree
  ...
  LICENSE
  ```
- [ ] `pyproject.toml` with pinned deps (fastapi, uvicorn, pvlib, httpx, pydantic, redis, pytz, numpy, pandas).
...
- [ ] Create time index for [now → +6 days] at selected cadence (local tz).
- [ ] Solar position via pvlib SPA.
- [ ] Clear-sky irradiance (Ineichen + Linke turbidity).
- [ ] Transposition to plane (Hay-Davies or Perez).
- [ ] Module temperature model (SAPM/NOCT with ambient fallback).
- [ ] DC → AC conversion with generic CEC module & Sandia inverter; cap at `kWp`; apply scalar system loss.
- [ ] Compute:
  - [ ] `watts` (AC W time-series)
  - [ ] cumulative `watt_hours`
  - [ ] daily totals `watt_hours_day`
- [ ] Package output to API schema.

---

## P0 — API layer

- [ ] FastAPI app with routers `/estimate`, `/clearsky`.
- [ ] Input validation: ranges for lat, lon, declination, azimuth, kWp.
- [ ] Error handling to Forecast.Solar-like `message` block.
- [ ] Add CORS (optional).
- [ ] `/health` endpoint.
...
- [ ] `Dockerfile` (slim Python base, non-root user, uvicorn worker).
- [ ] `docker-compose.yml` with:
  - [ ] `solar-api` (port 8080)
  - [ ] `redis` (7-alpine)
- [ ] `TZ` env wiring; graceful shutdown; healthcheck.
...
- [ ] Unit tests for schema compliance (keys, types).
